...
MongoClient.connect(url, (err, client) => {
  if (err)
    console.log('Cant connect to mongo, your database probably isnt running')
...
export function attach(server: express.Express) {
  server.get('/mongo-api/v1/test/:search', (req, res) => {
    ...
    collection
      .find({ zip: { $regex: _.escapeRegExp(search) } })
      .limit(10)
      .toArray((err, docs) => {
  ...
  server.get(
    '/minimongo-api/get-total-population',
